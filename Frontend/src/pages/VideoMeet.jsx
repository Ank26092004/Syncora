...
export default function VideoMeetComponent() {
    ...
    useEffect(() => {
        ...
        const onKey = (e) => {
            ...
            switch (e.key.toLowerCase()) {
                ...
                case "s": if (screenAvailable) handleScreen(); break;
                case "c": toggleChat(); break;
                case "p": toggleParticipants(); break;
                case "k": setShowCaptions((s) => !s); break;
                default: break;
            }
        };
    ...
    // Live captions from the browser speech engine (presentation layer only)
    useEffect(() => {
        ...
        recognition.onresult = (event) => {
            ...
            for (let i = event.resultIndex; i < event.results.length; i++) {
                ...
                if (res.isFinal) {
                    setCaptions((prev) => [
                        ...prev.slice(-40),
                        { id: `${Date.now()}-${i}`, speaker: username || "You", text: res[0].transcript.trim() },
                    ]);
                } else {
                    ...
                    >
                        ...
                        <div className="absolute inset-x-0 bottom-0 flex items-center justify-between gap-3 bg-gradient-to-t from-[var(--scrim)] to-transparent px-4 py-3">
                            ...
                            <span className="flex items-center gap-2">
                                <ControlButton onClick={handleAudio} active={audio} label={audio ? "Mute microphone" : "Unmute microphone"}>
                                    {audio ? <Mic className="h-5 w-5" /> : <MicOff className="h-5 w-5" />}
                                </ControlButton>
                                <ControlButton onClick={handleVideo} active={video} label={video ? "Turn camera off" : "Turn camera on"}>
                    ...
                    >
                        ...
                        >
                            <div className="space-y-2">
                                <Label htmlFor="lobby-username" className="text-ink-soft">Display name</Label>
                                <Input
                                    id="lobby-username"
                                    value={username}
                ...
                {controlsVisible && (
                    ...
                    >
                        ...
                        <div className="flex items-center gap-2">
                            <span className={`hidden items-center gap-2 rounded-full border border-hairline px-3 py-1.5 text-xs text-ink-muted ${panelOpen ? "xl:flex" : "sm:flex"}`}>
                                <Keyboard className="h-3.5 w-3.5 shrink-0" aria-hidden="true" />
                                M mic · V cam · S share · C chat · P people · K captions
                            </span>
                            <ThemeToggle />
                        </div>
                    </motion.header>
                ...
                >
                    <AnimatePresence mode="popLayout">
                        ...
                        {videos.map((v) => (
                            <ParticipantTile
                                key={v.socketId}
                                label={`Guest ${String(v.socketId).slice(0, 4)}`}
                                stream={v.stream}
            ...
            <AnimatePresence>
                {controlsVisible && (
                    ...
                    >
                        ...
                        >
                            <ControlButton onClick={handleAudio} active={audio} label={audio ? "Mute microphone" : "Unmute microphone"} shortcut="M">
                                {audio ? <Mic className="h-5 w-5" /> : <MicOff className="h-5 w-5" />}
                            </ControlButton>
                            <ControlButton onClick={handleVideo} active={video} label={video ? "Turn camera off" : "Turn camera on"} shortcut="V">
                                {video ? <VideoIcon className="h-5 w-5" /> : <VideoOff className="h-5 w-5" />}
                            </ControlButton>
                            {screenAvailable === true && (
                                <ControlButton onClick={handleScreen} label={screen ? "Stop sharing screen" : "Share screen"} shortcut="S">
                                    {screen ? <MonitorStop className="h-5 w-5" /> : <MonitorUp className="h-5 w-5" />}
                                </ControlButton>
                            )}
                            <ControlButton onClick={toggleChat} label="Chat" shortcut="C" badge={showModal ? 0 : newMessages}>
                                <MessageSquare className="h-5 w-5" />
                            </ControlButton>
                            <ControlButton onClick={toggleParticipants} label="Participants" shortcut="P">
                                <Users className="h-5 w-5" />
                            </ControlButton>
                            <ControlButton onClick={() => setShowCaptions((s) => !s)} label="AI captions" shortcut="K">
                ...
                <ul className="space-y-2">
                    <li className="flex items-center justify-between gap-3 rounded-2xl border border-hairline bg-[var(--fill)] px-3 py-2.5">
                        ...
                        <span className="flex shrink-0 items-center gap-2 text-ink-muted">
                            {audio ? <Mic className="h-4 w-4" aria-label="Mic on" /> : <MicOff className="h-4 w-4 text-destructive" aria-label="Mic off" />}
                            {video ? <VideoIcon className="h-4 w-4" aria-label="Camera on" /> : <VideoOff className="h-4 w-4 text-destructive" aria-label="Camera off" />}
                        </span>
                    </li>
                    {videos.map((v) => (
                        <motion.li
                        ...
                        >
                            <span className="min-w-0 truncate text-sm text-ink">Guest {String(v.socketId).slice(0, 4)}</span>
            ...
            >
                {!captionsSupported ? (
                    ...
                    <div aria-live="polite" className="space-y-3">
                        ...
                        {interimCaption && (
                            <p className="text-sm text-ink-muted italic">{interimCaption}</p>
                        )}
                    </div>
                )}
            </SidePanel>
